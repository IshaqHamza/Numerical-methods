...
def nonlin_fin_diff(f, f_y, f_y1, a: float, b: float, alpha: float, beta: float, W_0: vector, N: int, M: int)->list[tuple]:
    ...
    for _ in range(M):
        J = [[2 + (h**2)*f_y(a+h, W_0[0], (W_0[1] - alpha)/(2*h)), -1 + (h)*f_y1(a+h, W_0[0], (W_0[1] - alpha)/(2*h))/2] + [0 for _ in range(N-2)]]
        ...
        J.append([0 for _ in range(N-2)] + [-1 - (h)*f_y1(b-h, W_0[N-1], (beta - W_0[N-2])/(2*h))/2, 2 + (h**2)*f_y(b-h, W_0[N-1], (beta - W_0[N-2])/(2*h))])
...
print(nonlin_fin_diff(lambda x, y, y1: 4 + 0.25*x**3 - 0.125*y*y1, lambda x, y, y1: -0.125*y1, lambda x, y, y1: -0.125*y, 1, 3, 17, 43/3, vector([17 - 2*i/15 for i in range(1, 20)]), 19, 100))